...
import type { StepDefinition, TraceEvent, WorkflowContext, WorkflowDefinition } from "../types";
...
export class StepRunner {
    ...
    constructor(deps: StepRunnerDeps) {
        ...
        this.retryPolicyService = deps.retryPolicyService ?? new RetryPolicyService();
    ...
    async executeStep(
        ...
        if (this.stepConditionService.shouldSkip(stepDef, ctx)) {
            ...
            this.logger.info("Step skipped", { workflowId: workflow.id, step: stepDef.name });
            return ctx.current;
        }
        ...
        for (let attempt = 1; attempt <= maxAttempts; attempt++) {
            ...
            this.logger.info("Step started", { workflowId: workflow.id, step: stepDef.name, attempt });

            try {
                const executor = this.registry.get(stepDef.type) as StepExecutor;
                const output = await executor.execute(ctx.current, ctx, stepDef.config ?? {});
                ...
                this.logger.info("Step success", { workflowId: workflow.id, step: stepDef.name, attempt });
                return ctx.current;
            } catch (err) {
                lastError = err;
        ...
        return this.executeFallback(workflow, stepDef, ctx, recursionDepth, lastError);
    }
    ...
    private async executeFallback(
        ...
        if (recursionDepth > this.fallbackMaxRecursionDepth) {
            ...
            throw new Error(`Fallback recursion exceeded for step "${stepDef.name}". Last error: ${lastMessage}`);
        }
        ...
        this.logger.warn("Executing fallback step", { workflowId: workflow.id, step: stepDef.name });

        try {
            const output = await this.executeStep(workflow, stepDef.fallback!, ctx, recursionDepth + 1);
            this.traceRecorder.record({
                type: "step_fallback_success",
                workflowId: workflow.id,
                stepName: stepDef.name,
            });
            this.logger.info("Fallback step success", { workflowId: workflow.id, step: stepDef.name });
            return output;
        } catch (err) {
            const message = err instanceof Error ? err.message : String(err);
            this.traceRecorder.record({
            ...
            this.logger.error("Fallback step failed", { workflowId: workflow.id, step: stepDef.name, error: message });
            throw err;
        }
    }
}
